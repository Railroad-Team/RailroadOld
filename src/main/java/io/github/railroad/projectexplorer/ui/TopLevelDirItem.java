...
public class TopLevelDirItem extends DirItem {
    ...
    public void sync(PathNode tree, Path initiator) {
        ...
        if (parent != null) {
            ...
            raise(
                new NoSuchElementException("Parent directory for " + relPath + " does not exist within " + getValue()));
            ...
            if (tree.isDirectory()) {
                ...
                raise(
                    new IllegalArgumentException("Cannot replace top-level directory " + getValue() + " with a file"));
            }
        }
    }
